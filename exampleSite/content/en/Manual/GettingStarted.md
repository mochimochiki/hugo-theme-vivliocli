...
The theme includes an exampleSite that can be used as a template. Copy the necessary files from the exampleSite. The default `hugo.toml` generated by `hugo new site` is overwritten with the theme's `hugo.toml`.
...
Open `/hugo.toml` and comment out or delete the following line:
...
In addition, the top-level output format and delimiter can be set in `/hugo.toml`.
...
> For other config parameters, please refer to the [hugo.toml](./config.html) and the [Hugo documentation](https://gohugo.io/getting-started/configuration/), among others.
